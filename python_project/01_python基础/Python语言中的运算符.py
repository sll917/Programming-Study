...
比较运算符和逻辑运算符
比较运算符也称为关系运算符，包括==、!=、<、>、<=、>=，我相信大家一看就能懂。需要提醒的是比较相等用的是==，请注意这里是两个等号，因为=是赋值运算符，我们在上面刚刚讲到过。比较不相等用的是!=，跟数学课本中使用的≠
并不相同，Python 2 中曾经使用过<>来表示不等于，在 Python 3 中使用<>会引发SyntaxError（语法错误）。比较运算符会产生布尔值，要么是True，要么是False。
